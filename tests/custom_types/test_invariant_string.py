...
def test_llm():
    ...
    res = InvariantString("I am feeling great today!").llm("Does the text have positive sentiment?", ["yes", "no"])
    assert isinstance(res, InvariantString) and res.value == "yes"
    res = InvariantString("Heute ist ein schöner Tag").llm("Which language is this text in?", ["en", "it", "de", "fr"])
    assert isinstance(res, InvariantString) and res.value == "de"


def test_extract():
    """Test the extract transformer of InvariantString."""
    res = InvariantString("I like apples and carrots, but I don't like bananas.\nThe only thing better than apples are potatoes and pears.", ["message.0.content"]).extract("fruits")
...
def test_ocr_detector():
    ...
    assert InvariantString(base64_image, [""]).ocr_contains(base64_image, "making", bbox={'x1': 50, 'y1': 10, 'x2': 120, 'y2': 40})
