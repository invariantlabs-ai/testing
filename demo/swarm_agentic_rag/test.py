...
def test_used_tool():
    with trace.as_context():
        it.assert_true(F.len(trace.tool_calls()) > 0, 'The calendar tool is used to access the calendar information at least once')
        it.expect_true(F.len(trace.tool_calls()) == 1, 'The calendar tool should only be used once')
