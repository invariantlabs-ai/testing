...
def does_not_click_on_firefox_menu(trace: Trace):
    ...
    for tool_out in tool_outs:
        if type(tool_out["content"]) == InvariantImage:
            assert_false(tool_out["content"].ocr_contains("New tab") and tool_out["content"].ocr_contains("New window"))
...
def run_agent(prompt: str) -> Trace:
    if "annotate" in prompt:
        return Trace.from_explorer("mbalunovic/tests-1732714692", 1)
    elif "100 traces" in prompt:
        return Trace.from_explorer("mbalunovic/tests-1732714692", 2)
    elif "chats-about-food" in prompt:
        return Trace.from_explorer("mbalunovic/tests-1732714692", 3)
    elif "anthropic" in prompt:
        return Trace.from_explorer("mbalunovic/tests-1732714692", 4)
    elif "fastapi" in prompt:
        return Trace.from_explorer("mbalunovic/tests-1732714692", 5)
    elif "fibonacci" in prompt:
        return Trace.from_explorer("mbalunovic/tests-1732714692", 6)
...
def test_annotation():
    ...
    with trace.as_context():
        ...
        type_tool_calls = trace.tool_calls({"arguments.action": "type", "name": "computer"})
...
def test_anthropic():
    ...
    with trace.as_context():
        ...
        assert_true(file_text.contains("import anthropic") or file_text.contains("from anthropic import"))
        ...
        for tool_out in tool_outs:
            if tool_out["content"].contains("Dataset:"):
                ...
                dataset_name = re.search(r'Dataset: (\w+)', tool_out["content"].value).group(1)
...
def test_code_agent_fastapi():
    ...
    with trace.as_context():
        ...
        for tool_call, tool_out in tp:
            assert_false(tool_call["function"]["name"] == "bash" and tool_out.get("content", "").contains("Permission denied"))
        ...
        for tc in tool_calls:
            ...
            assert_true(cnt[file_text] <= 2, "At least 3 edits to the same file with the same text")
...
def test_fibonacci():
    trace = run_agent("""write me a python function compute_fibonacci(n) that computes n-th fibonacci number and test it on a few inputs""")
