...
class Range(BaseModel):
    ...
    def from_line(text: str, line: int, exact_match: Optional[str] = None) -> "Range":
        ...
        lines = text.split("\n")
        if line >= len(lines):
            raise ValueError(f"Line number {line} is out of bounds for text with {len(lines)} lines")

        if exact_match and exact_match in lines[line]:
            start = sum(len(line) + 1 for line in lines[:line]) + lines[line].index(exact_match)
